  ...
  public IImage apply(IImage input) {
    ...
    for (int r = 0; r < data.length; r++) {
      for(int c = 0; c < data[0].length; c++) {
        ...
        newData[r][c].add("red", new_color_red);
        newData[r][c].add("green", new_color_red);
        newData[r][c].add("blue", new_color_red);
        ...
        if (c != data[0].length - 1 ) {
          newData[r][c + 1].add("red", 7 / 16 * error_red);
          newData[r][c + 1].add("green", 7 / 16 * error_red);
          newData[r][c + 1].add("blue", 7 / 16 * error_red);
        }

        // add (3/16 * error) to pixel on the next-row-left (r+1,c-1)
        if (c != 0 && r != data.length - 1 ) {
          newData[r + 1][c - 1].add("red", 3 / 16 * error_red);
          newData[r + 1][c - 1].add("green", 3 / 16 * error_red);
          newData[r + 1][c - 1].add("blue", 3 / 16 * error_red);
        }

        // add (5/16 * error) to pixel below in next row (r+1,c)
        if (r != data.length - 1 ) {
          newData[r + 1][c].add("red", 5 / 16 * error_red);
          newData[r + 1][c].add("green", 5 / 16 * error_red);
          newData[r + 1][c].add("blue", 5 / 16 * error_red);
        }

        // add (1/16 * error) to pixel on the next-row-right (r+1,c+1)
        if (r != data.length - 1 && c != data[0].length - 1 ) {
          newData[r + 1][c + 1].add("red", 1 / 16 * error_red);
          newData[r + 1][c + 1].add("green", 1 / 16 * error_red);
          newData[r + 1][c + 1].add("blue", 1 / 16 * error_red);
        }
